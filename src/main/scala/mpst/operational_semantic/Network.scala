...
import mpst.syntax.Type.{Local, Participant, Action, ChannelQueue, LocalEnv}
...
object Network:
  object NetworkCausal:
    ...
    def next[A >: Action](localsWithParticipant: Set[(Participant, Local)], pending: ChannelQueue)(using environment: LocalEnv): Set[(A, Set[(Participant, Local)], ChannelQueue)] =
    ...
    private def nextEntry(localWithParticipant: (Participant, Local), pending: ChannelQueue)(using environment: LocalEnv): Set[(Action, (Participant, Local), ChannelQueue)] =
  ...
  object NetworkMultiset:
    ...
    def next[A >: Action](localsWithParticipant: Set[(Participant, Local)], pending: Multiset[Action])(using environment: LocalEnv): Set[(A, Set[(Participant, Local)], Multiset[Action])] =
      localsWithParticipant.flatMap {
        localWithParticipant => nextEntry(localWithParticipant, pending).map {
          case (nextAction, nextLocalWithParticipant, nextPending) =>
            (nextAction, localsWithParticipant - localWithParticipant + nextLocalWithParticipant, nextPending)
    ...
    private def nextEntry(localWithParticipant: (Participant, Local), pending: Multiset[Action])(using environment: LocalEnv): Set[(Action, (Participant, Local), Multiset[Action])] =
